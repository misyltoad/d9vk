...
namespace dxvk {
  ...
  inline void D3D9DeviceEx::UploadSoftwareConstantSet(void* pData, const SoftwareLayoutType& Src, const D3D9ConstantLayout& Layout, const ShaderType& Shader) {
    ...
    auto dstData = reinterpret_cast<uint8_t*>(pData);

    if (constSet.meta->maxConstIndexF)
      std::memcpy(dstData + Layout.floatOffset(),   Src.fConsts, constSet.meta->maxConstIndexF * sizeof(Vector4));
    if (constSet.meta->maxConstIndexI)
      std::memcpy(dstData + Layout.intOffset(),     Src.iConsts, constSet.meta->maxConstIndexI * sizeof(Vector4i));
    if (constSet.meta->maxConstIndexB)
      std::memcpy(dstData + Layout.bitmaskOffset(), Src.bConsts, Layout.bitmaskSize());
  }
